...
export const WeekdaysFilter = () => {
  ...
  return (
    <div className="shadow-border h-44 w-74 rounded-xl bg-neutral-50 p-2 text-neutral-900">
      <div className="shadow-border-sm flex h-full w-full flex-col items-center justify-center rounded-lg bg-white p-3">
        <div className="flex h-10 w-full items-center justify-between">
          ...
          <label className="relative inline-flex cursor-pointer items-center select-none">
            ...
            {/* Track */}
            <div className="shadow-border-sm h-4.5 w-8 rounded-full bg-neutral-200/80 transition duration-300 ease-in-out peer-checked:bg-green-500 peer-focus-visible:ring-1 peer-focus-visible:ring-neutral-400/50 peer-focus-visible:ring-offset-1 peer-disabled:cursor-not-allowed peer-disabled:opacity-40 dark:bg-green-500 dark:peer-checked:bg-neutral-100 dark:peer-focus-visible:ring-offset-green-600" />

            {/* Thumb */}
            <div className="shadow-border-sm absolute top-[3px] left-[3px] h-3 w-3 rounded-full bg-white transition-all duration-150 ease-in-out peer-checked:translate-x-3.5 dark:bg-neutral-950" />
          </label>
        </div>

        <div className="mb-2 flex h-17 w-full items-end justify-between gap-2 py-1">
          <button className="mb-0.5 cursor-pointer rounded-2xl border-neutral-300 px-3 py-1 text-sm">
            Repeat
          </button>

          <button className="flex w-full cursor-pointer items-center justify-between rounded-lg border border-neutral-200 px-3 py-1 text-sm">
            <span>Weekly</span>
      ...
      {days.map((day) => {
        ...
        return (
          <div key={day} className="h-8 w-8 overflow-hidden rounded-md">
            ...
            >
              {/* OFF */}
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-[10px]">
                {day}
              </div>

              {/* ON */}
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-neutral-200 bg-white text-[10px]">
                {day}
              </div>
            </motion.div>
          </div>
